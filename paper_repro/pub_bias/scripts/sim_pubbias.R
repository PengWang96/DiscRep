...
for (c_val in c_vec) {
  ...
  for (jj in 1:num_simulations) {
    ...
    for (j in 1:length(samplesize)) {
      curor <- or[j]
      cur_indnum0 <- (samplesize[j] * 2) * 0.6
      cur_indnum1 <- (samplesize[j] * 2) * 0.4

      while (TRUE) {
        pcontrol <- runif(1, min = 0.6, max = 0.9) # runif(1, min=0.1, max=0.5)
        ...
        est_p <- summary(glm(datay ~ datax,
          family = "binomial"
        ))$coefficient[2, c(1, 2, 4)]

        if (rbinom(1, 1, wipi(est_p[3], c_val)) == 1) { # || est_p[3] <= 0.05
  ...
  saveRDS(list(hat_beta = hat_beta, hat_sigma_sq = hat_sigma_sq),
    file = paste0("../output/hat_beta_sigma_sq_c_", c_val, ".rds")
  )
  # saveRDS(pubbiass,
  #   file = paste0("../output/pubbiass_c_", c_val, ".rds")
  # )
  ...
  saveRDS(p_values,
    file = paste0("../output/pvalue_c_", c_val, ".rds")
  )
  ...
  cl <- makeCluster(numCores)
  registerDoParallel(cl)
  sim_results_q <- foreach(x = 1:num_simulations, .combine = combine_results, .packages = "DiscRep") %dopar% {
    metropolis_hastings(N, r, m, hat_beta[x, ], hat_sigma_sq[x, ], k_vec = k_vec)
  }
  stopCluster(cl)

  p_values <- sim_results_q$p_value
  bar_betas <- sim_results_q$bar_beta
  sen <- sum(p_values <= 0.05) / length(p_values)

  saveRDS(p_values,
    file = paste0("../output/pvalue_cQ_", c_val, ".rds")
  )

  cat("Sensitivity (Egger test):", sum(sim_results$p_value <= 0.05) / length(sim_results$p_value), "\n")
  cat("Sensitivity (Q-based posterior-PRP):", sen, "\n")
}
...
# library(doParallel)
# library(metafor)
# load(paste0("../output/hat_beta_c_", c, ".rds"))
# load(paste0("../output/hat_sigma_sq_c_", c, ".rds"))
#
...
#
# saveRDS(p_values,
#      file = paste0("../output/pvalue_cEgger_", c, ".rds"))
...
#
# saveRDS(p_values,
#      file = paste0("../output/pvalue_cBegg_", c, ".rds"))
# ggplot(data.frame(p_value = p_values), aes(x = p_value)) +
#   geom_histogram(color="white", fill="#6186ad", bins=20, boundary=0, alpha=0.8) +
#   scale_x_continuous(limits = c(0.00, 1.00)) +
#   theme_bw() +
